...
file_path = r'c:\Users\Sajjad\Downloads\modern-portfolio-main\modern-portfolio-main\src\app\page.tsx'
...
hero_desc_old = """Independent consultant helping tech CEOs start, grow, and scale businesses.
            Expert in{" "}
            <span style={{ color: "#C5A059", fontWeight: 600 }}>
              strategy, B2B linking, and business backbone structuring
            </span>."""

hero_desc_new = """A passionate Freelance Business Development Manager and Author. Focused on driving growth, building strategic B2B partnerships, and sharing the journey through compelling storytelling.
...
content = content.replace(hero_desc_old, hero_desc_new)

# 4. Insert Profile Image
profile_image_old = """<div className="absolute inset-0 flex items-center justify-center">
                <User size={48} className="md:size-16 lg:size-20" style={{ color: "#C5A059", opacity: 0.5 }} />
              </div>"""

profile_image_new = """<Image
...
content = content.replace(profile_image_old, profile_image_new)

# 5. Data replacement
data_old = """const experiences = [
  {
...
archive_ui_old = """                    <p className="text-sm md:text-base mb-3 leading-relaxed" style={{ color: "#1B3022", opacity: 0.8 }} itemProp="description">
                      {exp.description}
                    </p>
                    <div className="flex flex-wrap gap-2">"""

archive_ui_new = """                    <p className="text-sm md:text-base mb-4 leading-relaxed" style={{ color: "#1B3022", opacity: 0.8 }} itemProp="description">
                      {exp.description}
                    </p>
                    {exp.image && (
...
content = content.replace(archive_ui_old, archive_ui_new)
...
content = content.replace(engine_ui_old, engine_ui_new)

# In Library featured project:
library_feat_old = """              <h4 className="text-xl md:text-2xl font-bold mb-2" style={{ color: "#F5F2ED" }}>
...
library_feat_new = """              <h4 className="text-xl md:text-2xl font-bold mb-2" style={{ color: "#F5F2ED" }}>
...
content = content.replace(library_feat_old, library_feat_new)

# Arsenal padding with sajjad (9) image
arsenal_head_old = """            <h2 
...
arsenal_head_new = """            <h2 
          ...
          <motion.div"""
content = content.replace(arsenal_head_old, arsenal_head_new)
